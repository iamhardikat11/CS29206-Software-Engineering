...
class Main {
    private static class Entity {
        ...
        {
            System.out.println("Name : " + this.name + " ID : " + String.valueOf(this.id));
        }
    ...
    protected class Manufacturer extends Entity{
        ...
        public void PrintProductList()
        {
            ...
            {
                this.ProductList.get(i).printEntity();
            }
        }
    }
    ...
    protected class DeliveryAgent extends Entity{
    ...
    Main()
    {
        ...
        this.deliverables = new ArrayList<DeliveryAgent>();
        this.products = new ArrayList<Product>();
    }
    void printEntites(int entity)
    {
        switch(entity)
        {
            case 1:
                for(Product i : this.products){i.printEntity();}
                break;
            case 2:
                for(Manufacturer i : this.manufacturers){i.printEntity();}
                break;
            case 3:
                for(Customer i : this.customers){i.printEntity();}
                break;
            case 4:
                for(Shop i : this.shops){i.printEntity();}
                break;
            case 5:
                for(DeliveryAgent i : this.deliverables){i.printEntity();}
                break;
            default:
    ...
    void task1()
    {
        ...
        switch(todo)
        {
            ...
            case 3:
                ...
                {
                    case 1:
                        this.products.removeIf(ob -> ob.id == ID_d);
                        break;
                    case 2:
                        this.manufacturers.removeIf(ob -> ob.id == ID_d);
                        break;
                    case 3:
                        this.customers.removeIf(ob -> ob.id == ID_d);
                        break;
                    case 4:
                        this.shops.removeIf(ob -> ob.id == ID_d);
                        break;
                    case 5:
                        this.deliverables.removeIf(ob -> ob.id == ID_d);
                        break;
                    default:
                        break;
                }
    ...
    void task2()
    {
        ...
        int ID_p = sc.nextInt();
        int index_m=-1;int index_p = -1;
        for(int i=0 ;i<this.manufacturers.size();i++){if(this.manufacturers.get(i).id == ID_m){index_m=i;break;}}
        for(int i=0 ;i<this.products.size();i++){if(this.products.get(i).id == ID_p){index_p=i;break;}}
        if(index_m == -1 || index_p == -1){System.out.println("-------Enter Valid Task!!------");}
        else
        {
    ...
    void task3()
    {
        ...
        int ID_p = sc.nextInt();
        int index_s = -1;int index_p=-1;
        for(int i=0;i<this.shops.size();i++){if(this.shops.get(i).id == ID_s){index_s=i;break;}}
        for(int i=0;i<this.products.size();i++){if(this.products.get(i).id == ID_p){index_p=i;break;}}
        if(index_s == -1 || index_p == -1){System.out.println("-------Enter Valid Task!!------");}
        else
        {
    ...
    void task4()
    {
        ...
        int ID_c = sc.nextInt();
        int index_c = -1;
        for(int i=0;i<this.customers.size();i++){if(this.customers.get(i).id == ID_c){index_c=i;break;}}
        if(index_c == -1){System.out.println("-------Enter Valid Task!!------");}
        System.out.print("Enter ID of Product : ");
        int ID_p = sc.nextInt();
        int index_p = -1;
        for(int i=0;i<this.products.size();i++){if(this.products.get(i).id == ID_p){index_p=i;break;}}
        if(index_p == -1){System.out.println("-------Enter Valid Task!!------");}
        int index_s = -1;
    ...
    void task5()
    {
        ...
        int ID = sc.nextInt();
        int index = -1;
        for(int i=0;i<this.customers.size();i++){if(this.customers.get(i).id == ID){index=i;break;}}
        if(index == -1){System.out.println("-------Enter Valid Task!!------");}
        else{this.customers.get(index).printProductList();}
        sc.close();
    }
    void task6()
    {
        ...
        int index = -1;
        for(int i=0;i<size;i++){if(this.shops.get(i).id == ID){index=i;break;}}
        if(index == -1){System.out.println("-------Enter Valid Task!!------");}
        else{this.shops.get(index).printInventory();}
        sc.close();
    }
    void task7()
    {
        ...
        int index = -1;
        for(int i=0;i<size;i++){if(this.manufacturers.get(i).id == ID){index=i;break;}}
        if(index == -1){System.out.println("-------Enter Valid Task!!------");}
        else{this.manufacturers.get(index).PrintProductList();}
        sc.close();
    }
